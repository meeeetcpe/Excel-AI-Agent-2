...
function start(manifestPath, appType, command) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const app = command.app || process.env.npm_package_config_app_to_debug;
            ...
            const sideload = command.sideload || process.env[`npm_package_scripts_sideload_${app}`] || process.env.npm_package_scripts_sideload;
            if (appTypeToDebug === undefined) {
                throw new Error("Please specify the application type to debug.");
            }
            yield start_1.startDebugging(manifestPath, appTypeToDebug, debuggingMethod, sourceBundleUrlComponents, devServer, devServerPort, packager, packagerHost, packagerPort, sideload, enableDebugging, enableLiveReload);
        }
        catch (err) {
...
function stop(manifestPath, command) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const app = command.app || process.env.npm_package_config_app_to_debug;
            const unload = command.unload || process.env[`npm_package_scripts_unload_${app}`] || process.env.npm_package_scripts_unload;
            yield stop_1.stopDebugging(manifestPath, unload);
        }
        catch (err) {
